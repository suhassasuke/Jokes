package com.example.jokes.connect
...
class JSONDownloader(private var c: Context, private var jsonURL: String, private var type: String, private var view: Any) : AsyncTask<Void, Void, String>() {
    ...
    override fun onPostExecute(jsonData: String) {
        ...
        if (jsonData.startsWith("URL ERROR")) {
            ...
            Toast.makeText(
                c,
                "MOST PROBABLY APP CANNOT CONNECT DUE TO WRONG URL SINCE MALFORMEDURLEXCEPTION WAS RAISED",
                Toast.LENGTH_LONG
            ).show()
            ...
            Toast.makeText(
                c,
                "MOST PROBABLY APP CANNOT CONNECT TO ANY NETWORK SINCE IOEXCEPTION WAS RAISED",
                Toast.LENGTH_LONG
            ).show()
        } else {
            //PARSE
            Toast.makeText(
                c,
                "Network Connection and Download Successful. Now attempting to parse .....",
                Toast.LENGTH_LONG
            ).show()
            if (type=="categories"){
                CategoryParser(c, jsonData, view as ViewPager).execute()
            }else{
                JokesParser(c,jsonData,view as RecyclerView).execute()
            }
        }
    }
